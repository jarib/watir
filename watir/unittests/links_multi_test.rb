...
class TC_Links_Multi < Test::Unit::TestCase
  ...
  def test_not_found_single
    ...
    assert_equal('Unable to locate Link, using :id and "Missing"', exception.message)
  end
  ...
  def test_not_found_with_multi
    ...
    assert_equal('Unable to locate Link, using {:class=>"Missing", :index=>2}', exception.message)
  end
